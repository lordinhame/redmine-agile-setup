...
module AgileBoardsHelper
  ...
  def render_board_fields_selection(query)
    query.available_inline_columns.reject(&:frozen?).reject{ |c| c.name == :story_points && !RedmineAgile.use_story_points? }.each_with_index.map do |column, index|
      label_tag("c_#{column.name}", check_box_tag('c[]', column.name, query.columns.include?(column), id: "c_#{column.name}") + column.caption, class: "floating")
    end.join(" ").html_safe
  end
  ...
  def render_board_fields_status(query)
    ...
    status_tags = available_statuses.map do |status|
      label_tag("f_status_#{status.id}", check_box_tag('f_status[]', status.id, current_statuses.include?(status.id.to_s), id: "f_status_#{status.id}") + status.to_s, class: 'floating')
    end.join(' ').html_safe
